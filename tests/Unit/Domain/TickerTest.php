...
class TickerTest extends TestCase
{
    ...
    private function indexAlreadyOverStopTestCases(): iterable
    {
        yield 'over SHORT stop (-)' => [
            'ticker' => TickerFactory::create(SymbolEnum::BTCUSDT, 200500, 100500),
    ...
    private function isLastPriceOverIndexPriceTestCases(): iterable
    {
        yield '[SHORT] last price over index price (+)' => [
            'ticker' => TickerFactory::create(SymbolEnum::BTCUSDT, 30100, 30090, 30099),
            'positionSide' => Side::Sell,
            'expectedResult' => true,
        ];

        yield '[SHORT] last price over index price (-)' => [
            'ticker' => TickerFactory::create(SymbolEnum::BTCUSDT, 30100, 30090, 300100),
            'positionSide' => Side::Sell,
            'expectedResult' => false,
        ];

        yield '[LONG] last price over index price (+)' => [
            'ticker' => TickerFactory::create(SymbolEnum::BTCUSDT, 30100, 30090, 30101),
            'positionSide' => Side::Buy,
            'expectedResult' => true,
        ];

        yield '[LONG] last price over index price (-)' => [
            'ticker' => TickerFactory::create(SymbolEnum::BTCUSDT, 30100, 30090, 30100),
            'positionSide' => Side::Buy,
            'expectedResult' => false,
        ];
    }
