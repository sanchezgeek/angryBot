...
final class CancelOrderResponseBuilder implements ResponseBuilderInterface
{
    ...
    private function __construct(private readonly ?ByBitV5ApiError $error, private readonly ?string $closedOrderId)
    {
    }

    public static function ok(string $closedOrderId): self
    {
        return new self(null, $closedOrderId);
    }
...
//        } else {
            $body = array_replace_recursive(self::SAMPLE_PLACE_ORDER_SUCCESS_RESPONSE, [
                'result' => ['orderId' => $this->closedOrderId]
            ]);
//        }
